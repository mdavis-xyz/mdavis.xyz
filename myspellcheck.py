...
def checkWord(word):
    ...
    if (word != '') and (word not in dictionary) and (word.lower() not in dictionary):
        ...
        print(f"Error: word {word} does not appear in the dictionary")
        if word != word.lower():
           print(f"   y - add, lowercase {word.lower()}")
           print(f"   Y - add, as is {word}")
        else:
           print(f"   y - add as is {word.lower()}")
...
def checkFile(fname):
    ...
    for (i,line) in enumerate(lines):
        if not checkLine(line):
            print("Quitting")
            print(f"That was file {fname} line {i+1}")
            print(line)
