...
This kind of data is sometimes called "bitemporal" data.
...
The purpose of MLF and TLF is to account for this.
...
`TOTALCLEARED`, `DISPATCH_UNIT_SCADA`, `UIGF` and `INITIALMW` all reflect power at the site, [before loss factors are applied](https://groups.google.com/g/nemosis-discuss/c/9YOmItTYc_Y/m/alILno39BQAJ). When looking at bidding data, bid volume (MWh) reflects power at the site, and prices reflect the price at the local node. You must multiply the bid price by loss factors to get the regional price.
...
This is a different concept to Australia's nodal dispatch, regional settlement process (where "local price" may differ from the regional price). For more information about that concept, see AEMO's [Guide to Mis-Pricing Information](https://www.aemo.com.au/-/media/files/electricity/nem/security_and_reliability/dispatch/policy_and_process/guide-to-mis-pricing-information.pdf)
