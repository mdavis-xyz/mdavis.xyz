...
Even if inspection was allowed, it is scientifically impossible to *confirm* that that any non-trivial electronic device has not been infected with malware.
...
For the first two options (which are paperless), it is *literally impossible* to detect fraud if it happens, unless you give up anonymity.
...
However it only takes [a dozen flipped votes](https://en.wikipedia.org/wiki/List_of_close_election_results) at a carefully chosen electorate to change the outcome of the whole election, which would not raise suspicion.
...
(e.g. "40% of visitors leave before scrolling to the bottom of the article.")
...
All software has bugs, typically around [ten to fifty bugs](https://www.mayerdan.com/ruby/2012/11/11/bugs-per-line-of-code-ratio) (coding mistakes) for every thousand lines of code.
...
The officials' response was to claim nothing was wrong, and to blame voters for not waiting "at least three to five seconds for all choices to be rendered on the eSlate voting machines".
...
For example, many voting machines have the candidate list loaded on using a SD card, which is inserted into all machines, one after the other.
It is [relatively simple](https://mirror.netcologne.de/CCC/congress/2014/webm-sd/31c3-6344-en-Security_Analysis_of_Estonias_Internet_Voting_System_webm-sd.webm) to construct a virus such that if you infect just one machine, then the virus will infect an inserted SD card, which will infect all subsequent machines programmed with that card.
